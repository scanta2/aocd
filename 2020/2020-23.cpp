...
constexpr long long seed[] = {3,6,4,2,9,7,5,8,1};
constexpr long long NUM_CUPS = 1000000;
constexpr long long TURNS = 10000000;

void do_turn(Link *current, vector<Link> &links, long long turn) {
    Link *pulled[3];
    long long taboo[3];
    {
...
int main() {
    ...
    cout << a->num << ", " << b->num << ", " << static_cast<long long>(a->num) * static_cast<long long>(b->num) << '\n';
